...
class MOjiScrollableScrollPhysics extends ScrollPhysics {
  ...
  const MOjiScrollableScrollPhysics({ ScrollPhysics parent }) : super(parent: parent);

  @override
  MOjiScrollableScrollPhysics applyTo(ScrollPhysics ancestor) {
    return MOjiScrollableScrollPhysics(parent: buildParent(ancestor));
  }
...
//      absDelta -= deltaToLimit;
  ...
  Simulation createBallisticSimulation(ScrollMetrics position, double velocity) {
    ...
    if (velocity.abs() >= tolerance.velocity || position.outOfRange) {//速度大于设定速度阈值或者发生overscroll时创建弹道模拟器
...
class MyBouncingScrollSimulation extends Simulation {
  ...
  MyBouncingScrollSimulation({
    ...
    if (position < leadingExtent) {//如果内容在最小滚动下面，也就是有underscroll
      _springSimulation = _underscrollSimulation(position, velocity);
      _springTime = double.negativeInfinity;
    } else if (position > trailingExtent) {//内容部分在最大滚动上面，也就是有overscroll
      _springSimulation = _overscrollSimulation(position, velocity);
      _springTime = double.negativeInfinity;
    } else {//在最小滚动和最大滚动之间，也就是没有overscroll和underscroll
      _frictionSimulation = FrictionSimulation(0.135, position, velocity);//摩擦减速模拟器，传入的值分别为：流体阻力系数、当前位置、当前速度
      final double finalX = _frictionSimulation.finalX;//计算出在摩擦阻力的情况下，最终停留的位置
      if (velocity > 0.0 && finalX > trailingExtent) {//如果向上滑动，且最终停留的位置会超出最大滚动位置
        _springTime = _frictionSimulation.timeAtX(trailingExtent);//计算到达指定位置需要的时间，这里是计算到达最大滚动位置需要的时间
        _springSimulation = _overscrollSimulation(
          trailingExtent,//开始的位置
          math.min(_frictionSimulation.dx(_springTime), maxSpringTransferVelocity),//计算到达最大滚动位时的速度和系统指定的最大速度，取较小的一个
        );
        assert(_springTime.isFinite);
      } else if (velocity < 0.0 && finalX < leadingExtent) {////如果向下滑动，且最终停留的位置会超出最小滚动位置
        //下面同理
